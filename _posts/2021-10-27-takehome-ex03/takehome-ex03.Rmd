...
### 4.9 Promixty to Kindergartens and Number of Kindergartens within 350m
...
### 4.10  Promixty to childcare centres and Numbers of childcare centres within 350m
...
### 4.11 Promixty to bus stop and Numbers of bus stop within 350m
...
```{r}
hdb_final_mlr <- lm(formula = resale_price ~ floor_area_sqm + remaining_lease_year + PROX_CBD + PROX_ELDERLYCARE + PROX_HAWKER + PROX_MRTLRT + PROX_PARKS + PROX_SUPERMARKETS + PROX_CLINICS + NUM_CHILDCARE_350M + NUM_PRISCH_1KM + `01TO03` + `04TO06` + `07TO09` + `10TO12` + `13TO15` + `16TO18` + `19TO21` + `22TO24` + `25TO27` + `28TO30` + `31TO33` + `34TO36` + `37TO39` + `40TO42` + `43TO45` + `46TO48`, data=hdb_final_sf)

summary(hdb_final_mlr)
```

From the above, we can see that there are some independent variables that are statistically not significant. Variables such as PROX_MRTLRT, PROX_SUPERMARKETS, PROX_CLINICS, NUM_PRISCH_1KM, 43TO45 and 46TO48 have p-value above 0.05 and are not statistically significant. We will exclude these variable when we calibrate the revised model.

The code chunk calibrate the revised model using lm function with variables that are not statistically significant removed. Ordinary leasst squares regression is then done by running ols_regress function of olsrr package.

```{r}
hdb_final_mlr1 <- lm(formula = resale_price ~ floor_area_sqm + remaining_lease_year + PROX_CBD + PROX_ELDERLYCARE + PROX_HAWKER + PROX_PARKS + NUM_CHILDCARE_350M + `01TO03` + `04TO06` + `07TO09` + `10TO12` + `13TO15` + `16TO18` + `19TO21` + `22TO24` + `25TO27` + `28TO30` + `31TO33` + `34TO36` + `37TO39` + `40TO42`, data=hdb_final_sf)
...
```{r}
ols_plot_resid_fit(hdb_final_mlr1)
```

#### 7.1.5 Test for Normality Assumption

```{r}
ols_plot_resid_hist(hdb_final_mlr1)
```
...
```{r}
mlr.output <- as.data.frame(hdb_final_mlr1$residuals)
```

```{r}
hdb_final_res_sf <- cbind(hdb_final_sf, hdb_final_mlr1$residuals) %>%
                        rename(`MLR_RES` = `hdb_final_mlr1.residuals`)
```
...
```{r}
lm.morantest(hdb_final_mlr1, nb_lw)
```
...
PROX_PARKS + NUM_CHILDCARE_350M + `X01TO03` + `X04TO06` + `X07TO09` + `X10TO12` + `X13TO15` + `X16TO18` + `X19TO21` + `X22TO24` +
`X25TO27` + `X28TO30` + `X31TO33` + `X34TO36` + `X37TO39` + `X40TO42`, data=hdb_final_sp, approach="CV", kernel="gaussian", adaptive=FALSE, longlat=FALSE)
```
...
PROX_PARKS + NUM_CHILDCARE_350M + `X01TO03` + `X04TO06` + `X07TO09` + `X10TO12` + `X13TO15` + `X16TO18` + `X19TO21` + `X22TO24` +
`X25TO27` + `X28TO30` + `X31TO33` + `X34TO36` + `X37TO39` + `X40TO42`, data=hdb_final_sp, bw=bw.fixed, kernel="gaussian", adaptive=FALSE, longlat=FALSE)
```
...
PROX_PARKS + NUM_CHILDCARE_350M + `X01TO03` + `X04TO06` + `X07TO09` + `X10TO12` + `X13TO15` + `X16TO18` + `X19TO21` + `X22TO24` +
`X25TO27` + `X28TO30` + `X31TO33` + `X34TO36` + `X37TO39` + `X40TO42`, data=hdb_final_sp, approach="CV", kernel="gaussian", adaptive=TRUE, longlat=FALSE)
```
...
PROX_PARKS + NUM_CHILDCARE_350M + `X01TO03` + `X04TO06` + `X07TO09` + `X10TO12` + `X13TO15` + `X16TO18` + `X19TO21` + `X22TO24` +
`X25TO27` + `X28TO30` + `X31TO33` + `X34TO36` + `X37TO39` + `X40TO42`, data=hdb_final_sp, bw=bw.adaptive, kernel="gaussian", adaptive=TRUE, longlat=FALSE)
```
