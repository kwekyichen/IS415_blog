...
- HDB Resale data is a list of hdb resale transacted prices in Singaporefrom Jan 2017 onwards. It is in csv format from Data.gov.sg.
...
- get the nearest Elderlycare distance for each hdb row using rowwise function of dplyr to check through a row and get the min across the row with X133 column, and create a Min column using mutate function of dplyr package. 
...
From the above interactive plot, we can see that the residuals are not randomly distributed as the residual points seems to be clustered, with many of the same colour near each other. This means that there is a sign of spatial autocorrelation.
...
The following are the arguements: 
...
- convert the csv file into sf data frame using st_as_sf
...
From the above, we can see the gwr outputs such as Local_R2, residuals, coefficient standard error, and observed and predicted y values. We will visualise some of them in the next section. 
