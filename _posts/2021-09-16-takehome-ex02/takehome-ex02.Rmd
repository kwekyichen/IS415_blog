...
From the result above, there are no duplicate in mrtlrt_ppp object. The other ppp object have duplicates.
...
3. tmap_mode is set to view to plot interactive map
...
7. tmap_mode is set to view to plot interactive map
...
Analysis: From the raster map, we can see that areas such as Geylang and Clarke Quay have highest density of hotels with about 0.00015 to 0.00020 units per square meter. Areas like Rochor, Chinatown and Kallang have high density of about 0.00005 to 0.00015 units per square meter.
...
Analysis: From the raster map above, we can confirm that North-east, North-west and Central south area have higher density of Mrt and Lrt of about 0.000002 to 0.000004 units per square meter, as shown by the purple grid on the map.
...
Analysis: From the raster map above, we can confirm the the south-east area near central region such as City Hall have highest density of tourist attraction with about 0.000020 to 0.000025 units per square meter as shown by the dark purple grid on the map.
...
From the above, we can see that the common area with high density (in red and yellow) of Airbnb, hotel, mrt and tourist attraction are around the south east area near the central region.
...
Analysis: Similar to Kallang, even though there are already quite a number of hotels in Geylang and only one tourist attraction, there are still many Airbnb. There are 10 Mrt stations, which may be the reason why there are many Airbnb located in this area.
...
Analysis: Even though there are already a few hotels in Outram, there are still many Airbnb. The accessibility to the four MRT and multiple tourist attractions are probably the reason why Outram have many Airbnb. In addition, we can see that the area with most Airbnbs (black patch) area is also where the tourist attractions are located.
...
Analysis: In Downtown core, there are still many airbnb despite many hotels already available. The accessibility to many MRTs and multiple tourist attractions are probably the reason that many airbnb are located in this area.

Overall, we can conclude that there are no direct relationship between airbnb and hotel as we see that regardless of the number of hotel available in a region, the number of airbnb remained high. Tourist attractions may have some relationship with Airbnb, however, areas like Kallang and Geylang still have many Airbnb even though they only have one tourist attraction in the area. The accessibility of MRT is one of the possible factor that affects the number of Airbnb in the area, but mrt are easily accessbile almost every in Singapore. Hence, there are no direct relationship between Airbnb and Hotel, Mrt and Tourism respectively.
...
Analysis: R = 0.36 is less than 1, which means the distribution of Airbnb in Singapore in 2019 are likely clustered. The p-value of 0.01 is smaller than 0.05. Hence, we reject the null hypothesis that the Airbnb point patterns in Singapore are randomly distributed.
...
The following code chunk is similar to 5.2.1.2.2
...
Overall, we can conclude that airbnb in the four areas are not randomly distributed. For further analysis, we will narrow down to two areas, Downtown Core and Outram, with R value above 0.58 and 0.72 respectively. When R is nearer to 1, the distribution are likely to be random. Hence, we are interested to further analyse the distribution of Airbnb within these two region.
...
Analysis: The observed G(r) is above the G(theo) and higher convidence envelopes which means that Airbnb in Downtown Core area are clustered. Hence, we reject the null hypothesis that Airbnb in Downtown Core are randomly distributed at 99.9% confident interval.
...
Analysis: The observed G(r) is above the G(theo) and higher confidence envelopes which means that Airbnb in Outram area are clustered. Hence, we reject the null hypothesis that Airbnb in Outram are randomly distributed at 99.9% confident interval.
...
Analysis: The observed F(r) is below F(theo) and the lower confidence envelope, which means that Airbnb in Downtown Core are in clustered pattern. Hence, we reject the null hypothesis that Airbnb in Downtown Core are randomly distributed at 99.9% confident interval.
...
Analysis: The observed F(r) is below F(theo) and the lower confidence envelope, which means that Airbnb in Outram are in clustered pattern. Hence, we reject the null hypothesis that Airbnb in Outram are randomly distributed at 99.9% confident interval.
...
Analysis: The above L function plots shows that the observed L(r) is greater than L(theo) and the higher confidence envelope, which means clustering is statistically significant. This means Airbnb in Downtown Core are clustered. Hence, we reject the null hypothesis that Airbnb in Downtown Core are randomly distributed at 99.9% confident interval.
...
Analysis: The above L function plots shows that the observed L(r) is greater than L(theo) and the higher confidence envelope, which means clustering is statistically significant. This means Airbnb in Outram are clustered. Hence, we reject the null hypothesis that Airbnb in Outram are randomly distributed at 99.9% confident interval.
...
Analysis: The above density map shows a clearer visualization of the density of Airbnb in 2021 using grid. We can see that the Airbnb are scattered all around as seen by the lighter blue grid on the map in 2021. The south-east area near the central have higher density with around 200 - 800 units per square kilometer, represented by the brighter blue and pink grid on the map.
...
From the KDE map, the south-east region still have higher density of Airbnb in 2021. However, we noticed that the density of Airbnb have reduced as seen from the scale. It reduced from range 0-1500 units per square kilometer in 2019 to range 0-1000 units per square kilometer in 2021. We can conclude that Covid-19 did impact the Airbnb listing in Singapore from 2019 to 2021.
...
All ppp object except airbnb0619_pte_ppp have duplicates. 
...
1. The following code chunk computes kernel density using density function of spatstat package.
2. The code first takes in the ppp object variable in m.
3. bw.diggle automatic bandwidth method to detect a tight cluster
4. smoothing kernel is set to default gaussian method
...
The following code chunks plot KDE map of Airbnb 2019 and 2021 by each room type.
...
Analysis: In 2019, only Chinatown have highest density of private with 0.0008 to 0.0010 units per square m as shown in dark purple. In 2021, Kallang and Jalan Besar are new areas with high density 2021 with 0.0004 to 0.0006 units per square m. Chinatown's private room density became lower than Kallang's private room density in 2021.
...
Analysis: In 2019, area with higher density of entire home room type (shown in darker purples) are Anson and Geylang with density of 0.0008 to 0.0014 units per square m. In 2021, areas with higher density of entire home room are Geylang, Chinatown, Whampoa, Anson and Katong with density of 0.0002 to 0.0004 units per square m. 
...
Analysis: In 2019, area with higher density of shared room type (shown in darker purples) are Clarke Quay with density of 0.0004 to 0.0005 units per square m. In 2021, areas with higher density of shared room are Rochor, Jalan Besar and Bencoolen with density of 0.00006 to 0.00010 units per square m.
...
Analysis: Hotel room is a new room type in 2021. Chinatown is the only area with highest density of Hotel room with 0.00030 to 0.00035 units per square m in 2021.
...
Analysis: From the static raster map, we can see that there are lesser purple area in 2021 which means that there are lesser private room Airbnb from 2021 as compared to 2019 in Outram area. 
...
Analysis: From the static raster map, we can see that there are not much changes for the entire home Airbnb in Outram area from 2019 to 2021.
...
Analysis: From the static raster map, we can see that there are lesser purple area in 2021 which means that there are lesser shared room Airbnb from 2021 as compared to 2019 in Outram area. 
...
The following code chunk checks the data type of airbnb0619 SpatialPointsDataFrame

```{r}
str(airbnb0619)
```

We can see that the room_type are not in factor data type. 

The following code chunk converts room_type into a factor data type using as.factor function of base R package.
...
From the statistics above, Private room have the highest density of about 176 units per square kilometer. Entire home/apt have a density of about 118 units per square kilometer. Shared room have a density of about 49 units per square kilometer.

```{r}
intensity(rescale(airbnb0721rt_ot_ppp, 1000))
```

From the statistics above, Private room have the highest density of about 134 units per square kilometer. Entire home/apt have a density of about 88 units per square kilometer. Hotel room have a density of about 53 units per square kilometer. Shared room have a density of about 7 units per square kilometer.

### 6.2.2 Cross L-Function

Cross-type L-function computes the estimate of the cross-type K-function, and then applies the square root transformation. We will be looking at the relationship of different room types Airbnb located in the study area (Out), in 2019 and 2021. We have set.seed(1234) at the global setting so that the output will be the same.
...
Analysis: The plot above reveals that there is a sign that the marked spatial point events are not independent spatially from distance 0 to 150m as the L(r) is above 0. From 150m and above, there is a sign that the marked spatial point events are independent spatially as the L(r) is below 0. Hypothesis test is required to confirm the observation statistically.
...
Analysis: The plot above reveals that there is a sign that the marked spatial point events are not independent spatially from distance 20 to 120m, 170m to 200m as the L(r) is above 0. From 0 to 20m, 120m to 170, and 200 meter and above, there is a sign that the marked spatial point events are independent spatially as the L(r) is below 0. Hypothesis test is required to confirm the observation statistically.
...
Analysis: The plot above reveals that there is a sign that the marked spatial point events are not independent spatially from distance 5m and above as the L(r) is above 0. From 0 - 5m, there is a sign that the marked spatial point events are independent spatially as the L(r) is below 0. Hypothesis test is required to confirm the observation statistically.
...
Analysis: The plot above shows that the observed L(r) line is above L(theo) from about 5m onwards and is within the higher confidence envelope which means spatial clustering for that distance is statistically not significant. Hence, we failed to reject the null hypothesis that the distribution of Shared Room Airbnb and Entire home/apt Airbnb in 2021 are spatially independent at 99.9% confident interval.
...
Analysis: The plot above shows that the observed L(r) line is above L(theo) from around 10m ownwards and is within the higher confidence envelope which means spatial clustering for that distance is statistically not significant. Hence, we failed to reject the null hypothesis that the distribution of Shared Room Airbnb and Hotel Room Airbnb in 2021 are spatially independent at 99.9% confident interval.
...
Analysis: The plot above reveals that there is a sign that the marked spatial point events are not independent spatially from distance 5 to 130m as the L(r) is above 0. From 0 to 5 meter, and 130m and above, there is a sign that the marked spatial point events are independent spatially as the L(r) is below 0. Hypothesis test is required to confirm the observation statistically.
