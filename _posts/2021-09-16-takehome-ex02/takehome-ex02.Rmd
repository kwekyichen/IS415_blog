...
```{r}
packages = c('maptools', 'sf', 'raster','spatstat', 'tmap', 'tidyr', 'readr', 'rosm')
for (p in packages){
...
3. bw.diggle automatic bandwidth method to detect a tight cluster
4. smoothing kernel is set to default gaussian method
5. plot function of base R package is used to plot kde_airbnb0619SG_bw.km
...
plot(airbnb0619SG_ppp, main="Point Map")
```
...
##### 5.1.1.5.2 KDE Map and Point Map
...
### 5.1.2 Kernel density maps on Openstreetmap of Singapore to see possible relationship
...
Exploration of central region
```{r}
centralairbnb0619 <- airbnb0619_sf[airbnb0619_sf$neighbourhood_group == "Central Region",]
centralmpsz <- mpsz_sf[mpsz_sf$REGION_N == "CENTRAL REGION",]

tmap_mode("view")
tmap_options(check.and.fix = TRUE)
tm_shape(centralmpsz)+
  tm_polygons()+
...
```{r}
unique(centralairbnb0619$"neighbourhood")
```

Kallang, River Valley, Novena, Rochor
...
COMBINING AIRBNB0619 POINTS, HOTELS, MRT, TOURIST ATTRACTIONS AND THE STUDY AREA
...
There are also no direct relationship between airbnb and hotel, mrt and tourist attraction.
It would not make sense to plot density because there are some missing oberservations in some areas, for example, there is not tourist attractions in Kallang.
