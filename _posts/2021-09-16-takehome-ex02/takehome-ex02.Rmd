...
The following code chunk converts grid object into raster using raster function of raster package. Note that we will be using the non-rescaled gridded_kde_airbnb0619SG_bw.
...
From the Airbnb kde map above, we noticed that areas such as Kallang, Geylang, Outram and Downtown Core have a higher density as shown by the darker purple. Hence, we will look into these areas.
...
Overall, we can conclude that airbnb in the four areas are not randomly distributed. For further analysis, we will narrow down to two areas, Kallang and Outram because they have higher R as compared to the other area, with Outram having an R value above 0.7, which is nearer to 1. When R is nearer to 1, the distribution are likely to be random. Hence, we are interested to further analyse the distribution of airbnb within these two region.
...
Overall, Chinatown has most happening 
...
1. Extracts the target areas (Outram)
...
The following code chunk calculates KDE using density function of raster package
...
1. converts KDE output into grid object using as.SpatialGridDataFrame.im function of maptools package
...
airbnb0619_pte_ot_map <- tm_shape(kde_airbnb0619_pte_ot_bw_raster) + 
                  tm_raster("v", palette = "Purples", alpha = 0.7) +
                  tm_layout(legend.position = c("right", "bottom"), frame = FALSE) +
                  tm_basemap("OpenStreetMap")

airbnb0721_pte_ot_map <- tm_shape(kde_airbnb0721_pte_ot_bw_raster) + 
                  tm_raster("v", palette = "Purples", alpha = 0.7) +
                  tm_layout(legend.position = c("right", "bottom"), frame = FALSE) +
                  tm_basemap("OpenStreetMap")
...
airbnb0619_ent_ot_map <- tm_shape(kde_airbnb0619_ent_ot_bw_raster) + 
                  tm_raster("v", palette = "Purples", alpha = 0.7) +
                  tm_layout(legend.position = c("right", "bottom"), frame = FALSE) +
                  tm_basemap("OpenStreetMap")

airbnb0721_ent_ot_map <- tm_shape(kde_airbnb0721_ent_ot_bw_raster) + 
                  tm_raster("v", palette = "Purples", alpha = 0.7) +
                  tm_layout(legend.position = c("right", "bottom"), frame = FALSE) +
                  tm_basemap("OpenStreetMap")
...
airbnb0619_sh_ot_map <- tm_shape(kde_airbnb0619_sh_ot_bw_raster) + 
                  tm_raster("v", palette = "Purples", alpha = 0.7) +
                  tm_layout(legend.position = c("right", "bottom"), frame = FALSE) +
                  tm_basemap("OpenStreetMap")

airbnb0721_sh_ot_map <- tm_shape(kde_airbnb0721_sh_ot_bw_raster) + 
                  tm_raster("v", palette = "Purples", alpha = 0.7) +
                  tm_layout(legend.position = c("right", "bottom"), frame = FALSE) +
                  tm_basemap("OpenStreetMap")
...
```{r eval=FALSE}
intensity(rescale(airbnb0619SG_ot_ppp, 1000))
```

```{r eval=FALSE}
intensity(rescale(airbnb0619SG_pte_ot_ppp, 1000))
intensity(rescale(airbnb0619SG_ent_ot_ppp, 1000))
intensity(rescale(airbnb0619SG_sh_ot_ppp, 1000))
intensity(rescale(airbnb0721SG_pte_ot_ppp, 1000))
intensity(rescale(airbnb0721SG_ent_ot_ppp, 1000))
intensity(rescale(airbnb0721SG_sh_ot_ppp, 1000))
intensity(rescale(airbnb0721SG_ht_ot_ppp, 1000))
```
```
